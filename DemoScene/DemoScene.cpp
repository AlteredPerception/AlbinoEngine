...
bool DemoScene::initialize(AlbinoEngine::Engine& engine)
{
    ...
    lightMarker->setCastShadows(true);
    lightMarker->setReceiveShadows(true);
    ...
    cubeMesh2->setPosition(3.0f, 0.0f, 3.0f);
...
void DemoScene::update(AlbinoEngine::Engine& engine, float dt)
{
    ...
    if (engine.input().isLeftMouseDown())
    {
        int dx = engine.input().getMouseDeltaX();
        int dy = engine.input().getMouseDeltaY();
    ...
    //if (!engine.window().hasFocus() || GetForegroundWindow() != hwnd)
    //{
    //    m_wasDown = false;
    //    return; 
    //}
...
//    const float mouseSensitivity = 0.0035f;
 ...
 //   }
    ...
    movingLight.position = { 3.0f, 4.0f, 0.0f };
    ...
    if (keyDown('1')) {
        PointLight light1 = engine.getLightManager().getPointLight(0);
        if (light1.turnOn && light1.castsShadows) {
            light1.turnOn = false;
            light1.castsShadows = false;
        }
        else if (!light1.turnOn && !light1.castsShadows)
        {
            light1.turnOn = true;
            light1.castsShadows = true;
        }
        
        engine.getLightManager().setPointLight(0, light1);
    }
