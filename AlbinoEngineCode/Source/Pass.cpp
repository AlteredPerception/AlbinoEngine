...
namespace AlbinoEngine
{
	...
	void Pass::updateAndBindFrameLightingCB(EffectContext& fx)
	{
		...
		for (UINT i = 0; i < fx.numDirectionalLights && i < 2; ++i)
		{
			...
			cb.directionalLights[i].turnOn = fx.directionalLights[i]->turnOn;
			cb.directionalLights[i].castsShadows = fx.directionalLights[i]->castsShadows;
		}
		...
		for (UINT i = 0; i < fx.numPointLights && i < 4; ++i)
		{
			...
			cb.pointLights[i].castsShadows = fx.pointLights[i]->castsShadows;
			cb.pointLights[i].turnOn = fx.pointLights[i]->turnOn;
		}
